...
template<typename T> struct latticeboltzmann_aos_opt
{
  ...
  friend std::ostream& operator<<(std::ostream& os, latticeboltzmann_aos_opt<T> const& p)
  {
    return os << "(" << p.size()<< ")";
  }
  ...
  latticeboltzmann_aos_opt(int size_)
  :  nx(size_),ny(size_/2)
          ...
          ))
  {
    ...
    bc    = nt2::zeros(nt2::of_size(1, nx, ny), nt2::meta::as_<int>());
    alpha = nt2::zeros(nt2::of_size(1, nx, ny), nt2::meta::as_<int>());;
    ...
    bc(1,_(s1x,s2x-1),_(s1y,s2y-1)) = 1;
    ...
    // moment 1
    alpha(1, s1x-1, _(s1y,s2y-1)) += 1<<0;
    // moment 2
    alpha(1, _(s1x,s2x-1), s1y-1) += 1<<1;
    // moment 3
    alpha(1, s2x, _(s1y,s2y-1))   += 1<<2;
    // moment 4
    alpha(1, _(s1x,s2x-1),s2y)    += 1<<3;
    // // moment 5
    alpha(1, _(s1x-1,s2x-2),s1y-1) += 1<<4;
    alpha(1, s1x-1, _(s1y,s2y-2))  += 1<<4;
    // moment 6
    alpha(1, _(s1x+1,s2x),s1y-1) += 1<<5;
    alpha(1, s2x,_(s1y,s2y-2))   += 1<<5;
    // moment 7
    alpha(1, s2x,_(s1y+1,s2y))   += 1<<6;
    alpha(1, _(s1x+1,s2x-1),s2y) += 1<<6;
    // moment 8
    alpha(1, s1x-1,_(s1y+1,s2y)) += 1<<7;
    alpha(1, _(s1x,s2x-2),s2y)   += 1<<7;

    // outside the domain
    alpha(1, 2,_) = 1<<5 | 1<<2 | 1<<6;
    alpha(1, nx-1,_) += 1<<4 | 1<<0 | 1<<7;
    alpha(1, _,2) += 1<<3;
    alpha(1, _,ny-1) += 1<<1;
    alpha(1, _(3,nx-2),2) += 1<<6 | 1<<7;
    alpha(1, _(3,nx-2),ny-1) += 1<<5 | 1<<4;
...
NT2_REGISTER_BENCHMARK_TPL( latticeboltzmann_aos_opt, (float) )
{
 run_until_with< latticeboltzmann_aos_opt<T> > ( 10., 10
                                 , fixed(1024)
