...
namespace boost { namespace dispatch
{
  /*!
    ...
    functor is a generic, concept-dispatched polymorphic function object.
    Every function in Boost.Dispatch are defined in term of implementing a
    call strategy for functor. The main rationale is to centralize and homogenize
    the way functions with multiple possible implementation with respect to types,
    architectures and compilation phases are handled.

    @tparam Tag Function tag to dispatch to.
    @tparam EvalContext Tag defining how to perform said function call with
  ...
  **/
  template<class Tag, class EvalContext> struct functor
  {
    ...
    /*!
      Polymorphic function call operator using hierarchies to select its own
      implementation.

      @param  args Functor call arguments
      @return The result of the application of current function on args.
    **/
