...
struct test_conv
{
  ...
  void operator()()
  {
    res = nt2::conv ( data, filt, nt2::same_ );
  }

  friend std::ostream& operator<<(std::ostream& os, test_conv<T> const& p)
  {
...
NT2_REGISTER_BENCHMARK_TPL( test_conv, NT2_SIMD_REAL_TYPES )
{
  std::size_t data_min= args("min",128);
  std::size_t data_max= args("max",4096);
  std::size_t data_step= args("step",2);
  std::size_t filt_size= args("filter",3);

  run_during_with< test_conv<T> > ( 1.
