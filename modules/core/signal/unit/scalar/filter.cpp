...
NT2_TEST_CASE_TPL( small_filter, NT2_REAL_TYPES )
{
  const std::size_t data_size = 5;
  nt2::table<T, nt2::of_size_<1,data_size> > data = nt2::linspace<T>(T(1),T(data_size),data_size);
...
{
  const std::size_t data_size = 20;
  nt2::table<T, nt2::of_size_<1,data_size> > data = nt2::cons<T>(nt2::of_size(1,data_size), 0.7788, 0.4235, 0.0908, 0.2665, 0.1537, 0.2810, 0.4401, 0.5271, 0.4574, 0.8754
                                                                                          , 0.2548, 0.2240, 0.6678, 0.8444, 0.3445, 0.7805, 0.6753, 0.0067, 0.6022, 0.3868);
  const std::size_t size_filt = 5;

  nt2::table<T, nt2::of_size_<1,size_filt> > filt = nt2::cons<T>(nt2::of_size(1,size_filt), 0.5181, 0.9436, 0.6377, 0.9577, 0.2407);
  nt2::table<T, nt2::of_size_<1,data_size> > res;
  nt2::tie(res) = nt2::filter(filt,1,data);

  nt2::table<T, nt2::of_size_<1,data_size> > ref = nt2::cons<T>(nt2::of_size(1,data_size), 4.034962800000000e-01, 9.542910300000000e-01, 9.432988400000001e-1, 1.239675240000000
