...
namespace nt2
{
  ...
  /*!
    @brief Twotonmb generator

    Create an array full of the Twotonmb constant.

    @par Semantic:

    twotonmb() semantic depends of its parameters type and quantity:
    ...
    - For any Integer @c n and any type @c T, the following code:
      @code
      auto x = twotonmb(n, as_<T>());
      @endcode
      generates an expression that evaluates as a @size2d{n,n} table filled with
      the <tt>Twotonmb<T>()</tt> value.

    - For any Integer @c sz1,...,szN and any type @c T, the following code:
      @code
      auto x = twotonmb(sz1,...,szn, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{sz1,szn} table filled
      with the <tt>Twotonmb<T>()</tt> value.

    - For any Expression @c dims evaluating as a row vector of @c N elements
      and any type @c T, the following code:
      @code
      auto x = twotonmb(dims, as_<T>());
      @endcode
      generates an expression that evaluates as a @sizes{dims(1),dims(N)}
      table filled with the <tt>Twotonmb<T>()</tt> value.
    ...
    @return An Expression evaluating as an array of a given type and dimensions
            filled with the @c Twotonmb constant.

    @sa Twotonmb
  **/
  template<typename... Args, typename ClassName>
  details::unspecified twotonmb(Args const&... dims, ClassName const& classname);

  /// @overload
  template<typename... Args> details::unspecified twotonmb(Args const&... dims);

  /// @overload
  template<typename ClassName> ClassName::type twotonmb(ClassName const& classname);
